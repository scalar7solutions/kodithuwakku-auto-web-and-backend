      ...
      <div class="col-md-12">
        <div class="card mb-4">
          <div class="card-header pb-0">
            <h5>Vehicle Colors</h5>
            <p>Manage Vehicle Colors.</p>
          </div>
          <hr />
          <div class="card-body">
            ...
            <form id="formAccountSettings" @submit.prevent="submit">
              ...
              <div class="mt-2">
                <button type="submit" class="btn btn-main me-2" :disabled="form.processing">
                  {{  vehicle_color ? "Update" : "Save" }}
                </button>
                <Link class="btn btn-outline-danger" :href="route('vehicle.color')">Cancel</Link>
              </div>
            </form>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.vehicle_color) {
      this.form.id = this.vehicle_color.id;
      this.form.status = this.vehicle_color.status;
      this.form.name = this.vehicle_color.name;
  ...
  methods: {
    submit() {
      this.form.post(
        this.vehicle_color
          ? route("vehicle.color.update")
          : route("vehicle.color.storedata"),
        {
          onSuccess: () => {
            this.form.reset();
