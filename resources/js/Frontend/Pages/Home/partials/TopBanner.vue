  ...
  <div class="hero">
    <div class="hero-content">
      <div class="hero-heading">
        <h1>
          BROWSE. <span class="faded">COMPARE. BUY</span>
        </h1>
      ...
      <!-- <section class="hero-section">
        <div class="hero-content">
          <form id="searchForm" class="search-form" @submit.prevent="onSearch">


            <div class="search-row mt-n2">

              <div class="search-field custom-dropdown">
                <button type="button" @click="toggleMakeDropdown" class="dropdown-btn">
                  {{ selectedMake.label || 'Make' }}
                </button>

                <ul v-show="showMakeDropdown" class="dropdown-list wide-dropdown">
                  <li v-for="(make, index) in makes" :key="index" @click="selectMake(make)">
                    <div class="dropdown-item">
                      <span class="dropdown-text">{{ make.label }}</span>
                      <img v-if="make.media && make.media.length > 0" :src="make.media[0].original_url" height="20"
                        class="ps-2" alt="" />
                    </div>
                  </li>
                </ul>

                <input type="hidden" id="make" name="make" :value="selectedMake.value">
              </div>

              <div class="search-field custom-dropdown">
                <button type="button" @click="toggleModelDropdown" class="dropdown-btn">
                  {{ selectedModel.label || 'Model' }}
                </button>
                <ul v-show="showModelDropdown" class="dropdown-list wide-dropdown">
                   <li v-if="models.length === 0" class="dropdown-item disabled">
                    <span class="dropdown-text">No models available</span>
                  </li>
                  <li v-for="(model, idx) in models" :key="idx" @click="selectModel(model)">
                    <div class="dropdown-item">
                      <span class="dropdown-text">{{ model.label }}</span>
                    </div>
                  </li>
                </ul>
                <input type="hidden" id="model" name="model" :value="selectedModel.value">
              </div>

              <div class="search-field custom-dropdown">
                <button type="button" @click="toggleTypeDropdown" class="dropdown-btn">
                  {{ selectedType.label || 'Type' }}
                </button>
                <ul v-show="showTypeDropdown" class="dropdown-list wide-dropdown">
                  <li v-for="(type, idx) in vehicleTypes" :key="idx" @click="selectType(type)">
                    <div class="dropdown-item">
                      <span class="dropdown-text">{{ type.label }}</span>
                      <img :src="type?.media?.length > 0 ? type?.media[0].original_url : ''" height="20" class="ps-2"
                        alt="">
                    </div>
                  </li>
                </ul>
                <input type="hidden" id="model" name="model" :value="selectedType.value">
              </div>


              <div class="search-field">
                <input type="text" id="engineCC" placeholder="Engine CC (More Than)" v-model="form.engineCC" />
              </div>
